...
def process_effect_modifier_phantasmal_gain(
    ...
    if ModifierType.PHANTASMAL in target.modifier_map:
        modifier_data = target.modifier_map[ModifierType.PHANTASMAL]
        modifier_data.stacks_current = min(modifier_data.stacks_current + value, STACKS_MAX)

        return [], []

    target.modifier_map[ModifierType.PHANTASMAL] = ModifierData(
        IS_BUFF,
        True,
