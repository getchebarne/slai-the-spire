from src.game.core.effect import Effect
from src.game.core.effect import EffectTargetType
from src.game.core.effect import EffectType
from src.game.entity.card import CardColor
from src.game.entity.card import CardRarity
from src.game.entity.card import CardType
from src.game.entity.card import EntityCard
from src.game.factory.lib import register_factory
from src.game.types_ import CardUpgraded


_NAME = "Terror"
_COLOR = CardColor.GREEN
_COST = 1
_COST_PLUS = 0
_VULNERABLE = 99
_EXHAUST = True
_RARITY = CardRarity.UNCOMMON
_TYPE = CardType.SKILL


@register_factory(_NAME)
def create_card_terror(upgraded: CardUpgraded) -> EntityCard:
    if upgraded:
        return EntityCard(
            f"{_NAME}+",
            _COLOR,
            _TYPE,
            _COST_PLUS,
            _RARITY,
            [
                Effect(
                    EffectType.MODIFIER_VULNERABLE_GAIN, _VULNERABLE, EffectTargetType.CARD_TARGET
                )
            ],
            _EXHAUST,
        )

    return EntityCard(
        _NAME,
        _COLOR,
        _TYPE,
        _COST,
        _RARITY,
        [Effect(EffectType.MODIFIER_VULNERABLE_GAIN, _VULNERABLE, EffectTargetType.CARD_TARGET)],
        _EXHAUST,
    )
