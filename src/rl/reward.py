...
def compute_reward(
    view_game_state: slai.GameState,
    view_game_state_next: slai.GameState,
    game_over_flag: bool,
) -> float:
    if game_over_flag:
        if view_game_state_next.character.health <= 0:
            # Loss
            return -1
    ...
    # Upgrades
    num_upgrades = sum(view_card.upgraded for view_card in view_game_state.deck)
    num_upgrades_next = sum(view_card.upgraded for view_card in view_game_state_next.deck)
